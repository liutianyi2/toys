...
int main(int argc, char **argv){
    ...
    //int is_reuse_addr = 1, ret;
    //socklen_t sl = sizeof(int);
    //ret = setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &is_reuse_addr, sl);
    //printf("%d\n", ret);
    //setsockopt(listen_fd, SOL_SOCKET, SO_REUSEPORT, &is_reuse_addr, sl);
    //getsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &is_reuse_addr, &sl);
    //printf("%d\n", is_reuse_addr);
